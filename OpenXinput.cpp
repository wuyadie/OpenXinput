...
void WINAPI TranslateCapabilities(DeviceInfo_t* pDevice, GamepadCapabilities0101* pGamepadCapabilities, XINPUT_CAPABILITIES_EX* pCapabilities);
void WINAPI TranslateCapabilities(GamepadCapabilities0102* pGamepadCapabilities, XINPUT_CAPABILITIES_EX* pCapabilitiesEx);
...
HRESULT WINAPI EnumerateDevicesOnDeviceInterface(HANDLE hDevice, LPCWSTR DevicePath);
HRESULT WINAPI ProcessEnumeratedDevice(DeviceInfo_t* pDevice);

HRESULT WINAPI GetAudioInterfaceStringInfo(DeviceInfo_t* pDevice, WORD* VendorId, WORD* ProductId, BYTE* InputId);
HRESULT WINAPI GetDeviceInterfaceDetail(HANDLE hDev, PDO_DEVICE_INTERFACE_DATA DeviceInterfaceData, PDO_DEVICE_INTERFACE_DETAIL_DATA* ppDeviceInterfaceDetailData);
...
BOOL WINAPI IsDeviceInactiveOnBus(DeviceInfo_t* pDevice);
void WINAPI OnEnableSettingChanged(BOOL bEnabled);
bool WINAPI MinFillFromInterface(HANDLE hDevice, DeviceInfo_t* pDevice);
...
HRESULT WINAPI GetAudioDevice(DeviceInfo_t* pDevice, void* pParams, DWORD reserved);
...
HRESULT WINAPI GetBaseBusInformation(DeviceInfo_t* pDevice, void* pParams, DWORD reserved);
...
void WINAPI RemoveBusDevice(DWORD dwBusIndex);

HRESULT WINAPI IsDevicePresent(DeviceInfo_t* pDevice);
BYTE WINAPI IsBusDevicePresent(DeviceInfo_t* pDevice);
...
HRESULT WINAPI GetCapabilities(DeviceInfo_t* pDevice, XINPUT_CAPABILITIES_EX* pCapabilitiesEx);
HRESULT WINAPI GetBaseBusInformation(DeviceInfo_t* pDevice, XINPUT_BASE_BUS_INFORMATION* pBaseBusInformation);
HRESULT WINAPI GetBatteryInformation(DeviceInfo_t* pDevice, BYTE DeviceType, XINPUT_BATTERY_INFORMATION* pBatteryInformation);
HRESULT WINAPI GetAudioDeviceInformation(DeviceInfo_t* pDevice, XINPUT_AUDIO_INFORMATION* pAudioInformation);
...
HANDLE WINAPI CreateDeviceInfoList();

BOOL WINAPI GetClassDevs(HANDLE hDev, const GUID* pGuid, LPCWSTR pDeviceID, DWORD Flags);
BOOL WINAPI EnumDeviceInfo(HANDLE DeviceInfoSet, DWORD MemberIndex, PDO_DEVINFO_DATA DeviceInfoData);
BOOL WINAPI EnumDeviceInterfaces(HANDLE hDev, PDO_DEVINFO_DATA DeviceInfoData, const GUID* InterfaceClassGuid, DWORD MemberIndex, PDO_DEVICE_INTERFACE_DATA DeviceInterfaceData);
BOOL WINAPI GetDeviceInterfaceDetail(HANDLE DeviceInfoSet, PDO_DEVICE_INTERFACE_DATA DeviceInterfaceData, PDO_DEVICE_INTERFACE_DETAIL_DATA DeviceInterfaceDetailData, DWORD DeviceInterfaceDetailDataSize, PDWORD RequiredSize);
BOOL WINAPI DestroyDeviceInfoList(HANDLE hDeviceInfoList);
...
LPVOID WINAPI MemAlloc(DWORD dwBytes);
void WINAPI MemFree(LPVOID lpMem);

HANDLE WINAPI OpenDevice(LPCWSTR DevicePath, DWORD dwFlagsAndAttributes);
...
static decltype(XInputGetState)* g_pfnXInputGetState_Override = nullptr;
static decltype(XInputSetState)* g_pfnXInputSetState_Override = nullptr;
static decltype(XInputGetCapabilities)* g_pfnXInputGetCapabilities_Override = nullptr;
static decltype(XInputEnable)* g_pfnXInputEnable_Override = nullptr;
static decltype(XInputGetAudioDeviceIds)* g_pfnXInputGetAudioDeviceIds_Override = nullptr;
static decltype(XInputGetBatteryInformation)* g_pfnXInputGetBatteryInformation_Override = nullptr;
static decltype(XInputGetKeystroke)* g_pfnXInputGetKeystroke_Override = nullptr;

BOOL WINAPI Initialize();
BOOL WINAPI Close(bool ContinueHost);
...
// Global functions
/////////////////////////////////////////////////////
HRESULT WINAPI InitializeOverrides();
void WINAPI CleanupOverrides();
void WINAPI UnInitializeInputHost();

void EventWriteDllLoaded(HRESULT hr);
void EventWriteVidPid(WORD vendorId, WORD productId);

HRESULT WINAPI RegisterUtcEventProvider();
void WINAPI UnregisterUtcEventProvider();
...
HRESULT WINAPI EnumerateXInputDevices()
{
    ...
    while (1)
    {
        ZeroMemory(&DevinfoData, sizeof(SP_DEVICE_INTERFACE_DATA));
        DevinfoData.cbSize = sizeof(SP_DEVICE_INTERFACE_DATA);
        if (DriverComm::SetupDiWrapper::EnumDeviceInterfaces(hDeviceInfoList, nullptr, &Protocol::XUSB_INTERFACE_CLASS_GUID, MemberIndex++, (PDO_DEVICE_INTERFACE_DATA)&DevinfoData) == FALSE)
            break;

        if (GetDeviceInterfaceDetail(hDeviceInfoList, (PDO_DEVICE_INTERFACE_DATA)&DevinfoData, (PDO_DEVICE_INTERFACE_DETAIL_DATA *)&DevIfaceDetailData) >= 0)
        {
...
HRESULT WINAPI GetAudioDevice(DeviceInfo_t* pDevice, void* pParams, DWORD reserved)
{
    ...
    while (1)
    {
        ...
        if (DriverComm::SetupDiWrapper::EnumDeviceInfo(hDev, MemberIndex++, (PDO_DEVINFO_DATA)&DevInfoData))
        {
...
HRESULT WINAPI GetBaseBusInformation(DeviceInfo_t* pDevice, XINPUT_BASE_BUS_INFORMATION* pBaseBusInformation)
{
    ...
    if (OutBuffer.field_1D != 0)
    {
        ...
        pBaseBusInformation->field_E = OutBuffer.field_24;
    }
...
HRESULT WINAPI GetBatteryInformation(DeviceInfo_t* pDevice, BYTE DeviceType, XINPUT_BATTERY_INFORMATION* pBatteryInformation)
{
    ...
    else
    {
        ...
        if (DeviceType == XINPUT_DEVTYPE_GAMEPAD)
        {
            ...
            //if (XInputInternal::DeviceInfo::GetAudioDevice(pDevice, &apiParam, 4) < 0 || v7 == 0)
            {
...
BOOL WINAPI GetDeviceInterfaceDetail(HANDLE hDev, PDO_DEVICE_INTERFACE_DATA DeviceInterfaceData, PDO_DEVICE_INTERFACE_DETAIL_DATA DeviceInterfaceDetailData, DWORD DeviceInterfaceDetailDataSize, PDWORD RequiredSize)
{
...
DWORD WINAPI XInputGetState(_In_ DWORD dwUserIndex, _Out_ XINPUT_STATE* pState)
{
    ...
    {
        result = XInputGetStateEx(dwUserIndex, pState);
...
DWORD WINAPI XInputSetState(_In_ DWORD dwUserIndex, _In_ XINPUT_VIBRATION* pVibration)
{
    ...
    else
    {
        ...
        hr = XInputCore::ProcessAPIRequest(dwUserIndex, XInputInternal::DeviceInfo::g_pfnSetVibrationDispatcher, &apiParam, 1);
...
DWORD WINAPI XInputGetCapabilities(_In_ DWORD dwUserIndex, _In_ DWORD dwFlags, _Out_ XINPUT_CAPABILITIES* pCapabilities)
{
    ...
    {
        result = XInputGetCapabilitiesEx(1, dwUserIndex, dwFlags, &capabilitiesEx);
        if (result == ERROR_SUCCESS)
...
DWORD WINAPI XInputGetAudioDeviceIds(_In_ DWORD dwUserIndex, _Out_writes_opt_(*pRenderCount) LPWSTR pRenderDeviceId, _Inout_opt_ UINT* pRenderCount, _Out_writes_opt_(*pCaptureCount) LPWSTR pCaptureDeviceId, _Inout_opt_ UINT* pCaptureCount)
{
    DWORD result;
    HRESULT hr;
...
DWORD WINAPI XInputGetBatteryInformation(_In_ DWORD dwUserIndex, _In_ BYTE devType, _Out_ XINPUT_BATTERY_INFORMATION* pBatteryInformation)
{
    ...
    else
    {
        ...
        if (doApiCall)
        {
            ...
            hr = XInputCore::ProcessAPIRequest(dwUserIndex, XInputInternal::DeviceInfo::GetBatteryInformation, &apiParam, 2);
            result = XInputReturnCodeFromHRESULT(hr);
        }
    }
...
DWORD WINAPI XInputGetKeystroke(_In_ DWORD dwUserIndex, _Reserved_ DWORD dwReserved, _Out_ PXINPUT_KEYSTROKE pKeystroke)
{
    ...
    else
    {
        ...
        {
            result = ERROR_SUCCESS;
            hr = XInputCore::ProcessAPIRequest(dwUserIndex, XInputInternal::DeviceInfo::GetKeystroke, &apiParam, 3);
            if (hr >= 0)
        ...
        else
        {
            ...
            {
                hr = XInputCore::ProcessAPIRequest(i, XInputInternal::DeviceInfo::GetKeystroke, &apiParam, 3);
                if (result == ERROR_SUCCESS)
                {
...
DWORD WINAPI XInputGetStateEx(_In_ DWORD dwUserIndex, _Out_ XINPUT_STATE* pState)
{
    ...
    else
    {
        ...
        hr = XInputCore::ProcessAPIRequest(dwUserIndex, XInputInternal::DeviceInfo::g_pfnGetStateDispatcher, &apiParam, 1);
        result = XInputReturnCodeFromHRESULT(hr);
    }
    return result;
}
...
DWORD WINAPI XInputWaitForGuideButton(_In_ DWORD dwUserIndex, _In_ HANDLE hEvent, _Out_ XINPUT_LISTEN_STATE* pListenState)
{
    ...
    hr = XInputCore::ProcessAPIRequest(dwUserIndex, XInputInternal::DeviceInfo::WaitForGuideButton, &apiParam, 2);
...
DWORD WINAPI XInputCancelGuideButtonWait(_In_ DWORD dwUserIndex)
{
    ...
    hr = XInputCore::ProcessAPIRequest(dwUserIndex, XInputInternal::DeviceInfo::CancelGuideButtonWait, nullptr, 0);
    return XInputReturnCodeFromHRESULT(hr);
}
...
DWORD WINAPI XInputPowerOffController(_In_ DWORD dwUserIndex)
{
    ...
    hr = XInputCore::ProcessAPIRequest(dwUserIndex, XInputInternal::DeviceInfo::PowerOffController, nullptr, 0);
    return XInputReturnCodeFromHRESULT(hr);

}

DWORD WINAPI XInputGetBaseBusInformation(_In_ DWORD dwBusIndex, _Out_ XINPUT_BASE_BUS_INFORMATION* pBaseBusInformation)
{
    DWORD result;
    HRESULT hr;
...
DWORD WINAPI XInputGetCapabilitiesEx(_In_ DWORD dwReserved, _In_ DWORD dwUserIndex, _In_ DWORD dwFlags, _Out_ XINPUT_CAPABILITIES_EX* pCapabilitiesEx)
{
    ...
    hr = XInputCore::ProcessAPIRequest(dwUserIndex, XInputInternal::DeviceInfo::GetCapabilities, &apiParam, 1);
    result = XInputReturnCodeFromHRESULT(hr);
